...
**Acceptance Scenarios (BDD)**:
1. **Dado** uma role publicada na versao N, **Quando** o gestor altera permissoes ou atributos e envia via `/api/v1/roles/{id}` com `If-Match`, **Entao** cria-se versao N+1, a anterior permanece consultavel, requests sem `If-Match` valido sao rejeitadas com Problem Details e o contrato OpenAPI 3.1/Pact e atualizado.  
...
| Art. XI (Governanca de API) | Contrato-primeiro OpenAPI 3.1, erros RFC 9457, rate limiting `Retry-After`, idempotencia com `Idempotency-Key`, controle de concorrencia com `ETag`/`If-Match` e deduplicacao/TTL auditable | 429/`Retry-After`, `Idempotency-Key` com TTL/deduplicacao, `ETag`/`If-Match`, lint/diff OpenAPI/Pact e Problem Details padronizado. |
