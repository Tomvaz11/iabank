...
**RLS/pgcrypto (alvos de governança)**: aplicar políticas nomeadas `*_tenant_select/insert/update/delete` (default deny `tenant_id = current_setting('iabank.tenant_id')::uuid`) em inventário explícito e ordenado: Tenant, TenantStateTransition, TenantSecurityProfile, Role, RoleVersion, RoleBinding, SubjectAttribute, IdempotencyKeyRecord, AuthorizationDecisionLog, AuthRefreshToken, mais tabelas legadas de seeds/fundação com `tenant_id`. Migração expand/contract: criar tabelas com RLS off → backfill/índices `CONCURRENTLY` (tenant-first) → habilitar RLS/políticas → VALIDATE constraints. GUC único `iabank.tenant_id` em middleware/managers. Criptografar com pgcrypto campos sensíveis (`idp_metadata`, contatos, device_fp, segredos MFA) e usar hashes auxiliares para unicidade quando necessário.
...
- **Fase 2 – Middleware/Serviços (ordem sugerida)**:  
  1) Middleware `X-Tenant-Id` + GUC + managers tenant-first (`with_tenant` para Celery/cron).  
  2) Aplicar RLS/índices nas novas tabelas (inventário explícito) + validações.  
  ...
  5) OIDC/SAML + MFA TOTP + cadeia `AuthRefreshToken`/reuse.  
  6) Publisher WORM em eventos críticos.  
...
- Idempotência: Redis + Postgres (RLS) com `idempotency_key` (<=128), `key_hash`, `payload_hash`, `tenant_id`, `endpoint`, `response_snapshot` (JSONB máx. ~16KB), `status`, `response_code`, `expires_at` (min. 24h). Replays iguais retornam snapshot; divergentes → 409/422. Índices `(tenant_id, endpoint)` e `(tenant_id, key_hash)`; GC via cron/beat.
...
- Rotação HMAC: `TenantSecurityProfile` expõe `hmac_key_version`/`hmac_salt_version`/`rotates_at`; cache `hmac:{tenant}:{version}` invalidado na troca de versão; grace curto (~24h) para aceitar chave antiga; auditoria WORM da rotação com `tenant_id`, versão antiga/nova, `rotates_at`.
...
### WORM e Auditoria
- Payload mínimo: tipo (tenant_transition|role_version_publish|role_version_rollback|auth_refresh_reuse|abac_denied_critical|idempotency_conflict_high_risk), tenant_id, actor_id, trace/span, etag_before/after, status, hash. Assinatura SHA-256 + KMS/Vault, verificação pós-upload (fail-close), retenção ≥365d. Buckets S3 Object Lock por ambiente `worm-tenant-rbac-{env}`, retries exponenciais (3) e timeout curto; eventos críticos síncronos (fail-close).
...
- Rotas/headers específicos:  
  - Tenants (create/update/transition): `X-Tenant-Id` + assinatura, `Idempotency-Key`, `If-Match` em transições; exibir Problem Details/RateLimit/Retry-After.  
  - Roles/versions/bindings/subject-attributes: `Idempotency-Key` em mutações, `If-Match` em PATCH/publish, tratamento de 409/412.  
  - Auth token/refresh/revoke: `Idempotency-Key`, hash de fingerprint no cliente, cookies HttpOnly/Secure/SameSite=Strict.  
