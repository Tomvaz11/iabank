...
**Primary Dependencies**: Django 4.2 LTS, Django REST Framework 3.15, Celery 5.3 + Redis 7 (para seeds assíncronas e datasets de carga), `factory-boy` como catálogo único de factories, PostgreSQL 15 com `pgcrypto` e RLS multi-tenant, OpenTelemetry SDK + Sentry para observabilidade, ferramentas de carga (k6 ou Gatling equivalente) e Pact + OpenAPI 3.0.3 para contratos (alinhado a `contracts/api.yaml` e `specs/003-seed-data-automation/contracts/seed-data-openapi.yaml`; eventual migração para 3.1 será tratada em ADR/feature futura, como exceção consciente às diretrizes de OpenAPI 3.1 da Constituição/`AGENTS.md`, registrada em `Complexity Tracking`); alinhado a `BLUEPRINT_ARQUITETURAL.md` (§§3.1, 6, 26) e aos ADR-010/ADR-012.  
...
**Testing**: `pytest` + `pytest-django`, client de teste do DRF para testes de integração-primeiro, factories `factory-boy` reaproveitadas entre testes unitários/integração/contrato/carga, Pact para contratos da API `/api/v1`, testes de carga com k6 (ou Gatling equivalente) usando o script planejado `tests/performance/test_seed_data_load_profiles.js` e scanners de PII para validar anonimização (incluindo o teste planejado `tests/security/test_pii_scanner_seeds.py`); obedecendo Art. III (TDD) e Art. IV (integração-primeiro) e Art. IX (gates de qualidade/performance).  
...
**Comando `seed_data` (interface)**: A interface de linha de comando exposta por `seed_data` terá parâmetros bem definidos: `--env` (enum: `dev`, `hom`, `perf`, `dr`, `review`), `--profile` (enum: `small`, `medium`, `large` — combinados com `--env` para resolver um `SeedProfile.id` como `dev-small`), `--tenants` (lista de slugs de tenant separados por vírgula; omitido assume “todos os tenants ativos do ambiente”), `--mode` (enum: `sync`, `async`, `auto`), `--dry-run` (somente relatório, sem commit final) e `--allow-reuse` (habilita reaproveitamento de registros existentes sem duplicação). Invocações inválidas (por exemplo, `env` não suportado, `profile` inexistente, tenants desconhecidos) devem falhar com código de saída diferente de zero e mensagem clara; a saída padrão será textual, com resumo agregando `SeedRun`/`SeedRunEntityMetric` por entidade/tenant, enquanto detalhes ficam em logs estruturados/OTEL. Seeds nunca criam registros “sem dono”: toda inserção/atualização de dados de negócio ocorre sempre com `tenant_id` definido e sob políticas de RLS; executar `seed_data` sem `--tenants` apenas expande o escopo para “todos os tenants ativos”, sem violar a regra de falha explícita para tenants inexistentes descrita na `spec.md`.  
...
**Observabilidade**: Instrumentação de seeds/anonimização/datasets com OpenTelemetry e W3C Trace Context, gerando métricas (tempo/volume por entidade/tenant, taxa de falhas, consumo de recursos), traces e logs estruturados em JSON sem vazamento de PII, integrados a Sentry e aos exporters já definidos em ADR-012 e no runbook `docs/runbooks/observabilidade.md`; relatórios de execução por entidade/tenant serão correlacionáveis via IDs de trace/spans. Métricas de referência incluem, por exemplo, histogramas de duração de `SeedRun` por ambiente/perfil, contadores de registros criados/reutilizados por entidade/tenant e gauges de volumetria efetiva vs alvo (Q11), expostos via Prometheus com labels (`environment`, `tenant_slug`, `entity_name`, `profile_id`).  
**Segurança/Compliance**: Conformidade com LGPD e políticas internas (Art. XII, XIII, XVI): apenas dados sintéticos ou irreversivelmente anonimizados em ambientes não produtivos, nenhuma PII real em logs ou datasets de teste, mapeamento explícito de PII/PD por modelo, políticas de mascaramento fortes, uso de `pgcrypto`/Vault Transit quando aplicável, conforme ADR-010 e o runbook `docs/runbooks/seguranca-pii-vault.md`, e validação automatizada com scanners de PII e amostragem; produção limitada a seeds técnicas com change management registrado; fluxos de desprovisionamento de ambientes ou de descomissionamento de tenants devem disparar rotinas de cleanup que removem ou anonimizam datasets de teste e metadados associados (incluindo registros em `SeedRunEntityMetric`/`SeedDatasetSnapshot`) para esses escopos, com trilha de auditoria adequada para comprovar o cumprimento de políticas de retenção e direito ao esquecimento em ambientes não produtivos.  
...
- [x] **Art. XI - Governança de API**: Contratos OpenAPI 3.0.3 e Pact para `/api/v1` serão atualizados/estendidos em `contracts/` para cobrir cenários dependentes de seeds/datasets sintéticos, mantendo versionamento por URL (`/api/v1`) e diffs controlados; erros seguirão RFC 9457 e semântica de RateLimit com `429`, `Retry-After` e cabeçalhos `RateLimit-*`. Nesta feature mantemos explicitamente OpenAPI 3.0.3 como base instalada, conforme registrado em `Complexity Tracking`, deixando a migração para OpenAPI 3.1 para ADR/feature dedicada.  
...
|   |
|   `-- contracts/
|       `-- tests/                                 # Testes de contrato para `/api/v1` com datasets sintéticos (inclui, por exemplo, `tests/contracts/test_seed_data_openapi_usage.py` para cenários de seeds)
|
|-- config/                                        # Configurações de ambiente/feature flags e perfis de volumetria
contracts/                                         # OpenAPI 3.0.3, Pact e artefatos de contrato atualizados
tests/
...
|-----------|------------|-------------------------------------|
| Art. XI — Especificação menciona OpenAPI 3.1 para evidência de governança de API, enquanto esta feature F-11 mantém contratos existentes em OpenAPI 3.0.3 (incluindo `contracts/api.yaml` e `specs/003-seed-data-automation/contracts/seed-data-openapi.yaml`) | A base instalada do monorepo já está em OpenAPI 3.0.3 e a feature F-11 trata apenas de seeds/datasets sintéticos e de anotações de uso de contratos existentes; migrar todo o contrato `/api/v1` para 3.1 dentro desta feature criaria uma mudança transversal grande, com risco de impacto em outras features, fugindo do escopo de automação de seeds | Elevar F-11 para também executar a migração completa de `contracts/api.yaml` para OpenAPI 3.1 agora (junto com ajustes de lint, Spectral e Pact) implicaria uma refatoração ampla e potencialmente disruptiva; decidiu-se manter 3.0.3 nesta feature e tratar a migração para 3.1 em ADR/feature dedicada, preservando foco de F-11 em seeds/PII/DR, conforme alinhado no blueprint e na Constituição |
