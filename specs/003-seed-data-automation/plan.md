...
- **SeedDataset (schema/GC)**: Tabela RLS com colunas mínimas `tenant_id`, `environment`, `mode`, `manifest_hash_sha256`, `seed_run_id` (FK), `checkpoint_ref`, `dataset_hash`, `status`, `created_at/updated_at`, `expire_at` (TTL por modo) e `last_cleanup_at`; índices em `(tenant_id, environment, mode)` e `expire_at` para GC. CLI/API devem limpar o dataset do modo/tenant antes de reexecuções carga/DR; GC periódico (Argo/Celery) remove expirados por TTL sem tocar evidências WORM.  
...
- **BudgetRateLimit (schema)**: Tabela RLS com colunas `tenant_id`, `environment`, `mode`, `manifest_hash_sha256`, `seed_run_id` (FK), `rate_limit_limit`, `rate_limit_window_seconds`, `rate_limit_reset_at`, `rate_limit_remaining`, `budget_cap_brl`, `budget_alert_at_pct`, `budget_consumed_brl`, `cost_model_version`, `cost_estimated_brl`, `cost_actual_brl`, `reset_at` (TTL), `created_at/updated_at`; índices em `(tenant_id, environment, mode)` e `rate_limit_reset_at/reset_at`. Integra com o gate runtime (abort em estouro) e com o relatório WORM (somente registro; WORM não é apagado pelo GC).  
...
- **Relatório WORM**: payload JSON com campos obrigatórios `{run_id, tenant, environment, manifest_path, manifest_hash_sha256, schema_version, mode, reference_datetime, caps, rate_limit_usage, budget_usage, batches[], checkpoints[], errors[], otel_trace_id, otel_span_id, started_at, finished_at, outcome, cost_model_version, cost_estimated_brl, cost_actual_brl}`; `signature_hash` = `sha256` do payload; assinatura assimétrica via KMS/Vault (`signature_algo` = `RSA-PSS-SHA256` ou `Ed25519`) armazenada em `signature` com `key_id`/`key_version`. Destino: bucket S3 Object Lock `worm/seeds/<env>/<tenant>/<run_id>.json`, retenção mínima 1855 dias (30 dias + 5 anos); verificar assinatura após upload e antes de marcar `integrity_status=verified`, com retry exponencial (máx. 3) em upload/verify.
...
- **Assinatura/WORM operacional**: cliente Python `boto3` com `ObjectLockMode=COMPLIANCE` e `RetainUntilDate` ≥ 1855 dias; assinatura via Vault Transit path `transit/sign/seeds-worm` (key `seed-worm-report`) ou KMS equivalente por ambiente (`arn:aws:kms:...:key/seeds-worm`). Passos: (1) calcular `signature_hash`; (2) `vault write transit/sign/seeds-worm hash=<signature_hash>`; (3) upload com metadados `x-seeds-signature`, `x-seeds-key-id`; (4) verificar via `transit/verify` após upload. Falha em qualquer passo → abortar `SeedRun`.  
...
- **Budget por execução (FinOps)**: valores em BRL conforme `configs/finops/seed-data.cost-model.yaml` (mesma moeda do cost-model): dev/homolog até R$25, staging/carga até R$125; alertar em 80%, abortar em 100% (fail-closed) e exigir ajuste do manifesto.
