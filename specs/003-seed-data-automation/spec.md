...
- **FR-010a**: Seeds/factories DEVEM cobrir estados principais por entidade (ativo/bloqueado/inadimplente/cancelado, conforme enums de domínio) com proporções configuradas no manifesto por tenant/mode, evitando inflar volumetria fora do perfil declarado.
...
- **FR-010d**: Datas de contrato/vencimento/TTL nas seeds/factories DEVEM ser derivadas de um `reference_datetime` fixo em UTC declarado no manifesto por tenant/mode, garantindo determinismo/idempotência e consistência de cálculos financeiros (CET/IOF/juros).
- **FR-010e**: O manifesto por tenant/mode DEVE incluir `reference_datetime` obrigatório em ISO 8601 UTC (`...Z`); as datas derivam diretamente dele sem uso de `now()`/timezones locais para evitar drift e manter reprodutibilidade em CI/Argo/dev.
