...
- Q: Qual fallback usar em dev isolado se o Vault/FPE estiver indisponível? → A: Fallback apenas em dev isolado, ainda via key/salt no Vault dev; se Vault dev estiver indisponível, abortar (sem `.env`). CI/staging/perf/prod falham se fallback estiver ativo ou se o Vault não estiver disponível.
...
- Q: Qual escopo de execução do `seed_data` em CI/PR vs cargas/DR completas? → A: CI/PR roda apenas dry-run do baseline determinístico (`seed_data --profile`), validando PII/contratos/idempotência; cargas/DR completas rodam em staging e perf dedicados, agendadas em janela off-peak com manifestos versionados, caps de volumetria/budget e evidência WORM.
