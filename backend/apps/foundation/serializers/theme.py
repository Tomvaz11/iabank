...
def _validate_unknown_categories(categories: dict[str, dict[str, str]], errors: list[str]) -> None:
    for category in categories.keys():
        if category not in REQUIRED_CATEGORIES:
            errors.append(f'{category}: categoria desconhecida')


def _validate_category_tokens(category: str, tokens: object, errors: list[str]) -> None:
    if tokens is None:
...
def _validate_categories(categories: dict[str, dict[str, str]]) -> None:
    errors: list[str] = []
    _validate_unknown_categories(categories, errors)
    for category in REQUIRED_CATEGORIES:
        _validate_category_tokens(category, categories.get(category), errors)
    if errors:
        joined = '; '.join(errors)
        raise serializers.ValidationError({'categories': [f'TokenSchema inválido: {joined}']})
...
class TenantThemeSerializer(serializers.Serializer):
    ...
    def _collect_token_data(
        self, tokens: Iterable[TenantThemeToken]
    ) -> tuple[dict[str, dict[str, str]], dict[str, dict[str, object]], str | None, object | None]:
        ...
        return categories, wcag_report, version, generated_at

    def to_representation(self, instance: dict) -> dict:
        tenant = instance['tenant']
        tokens: Iterable[TenantThemeToken] = instance['tokens']

        categories, wcag_report, version, generated_at = self._collect_token_data(tokens)
