...
from .services.seed_manifest_validator import RateLimitDecision, SeedManifestValidator, SimpleRateLimiter
...
class SeedProfileValidateView(APIView):
    ...
    def post(self, request: HttpRequest) -> Response:
        ...
        manifest = request.data.get('manifest') if isinstance(request.data, dict) else None

        if missing := [name for name, value in [('X-Tenant-ID', tenant_slug), ('X-Environment', environment), ('Idempotency-Key', idempotency_key)] if not value]:
        ...
        if rate_limit_decision and not rate_limit_decision.allowed:
            response = self._problem_response(
