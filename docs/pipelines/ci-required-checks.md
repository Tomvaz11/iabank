...
3. **tests-contract**: Spectral, oasdiff, Pact (`.github/workflows/ci-contracts.yml`) com cenários de idempotência e concorrência.
...
- Gate trunk-based: branches curtas (feature/*, release/* ou 3xx-*), squash-only/linear history (branch protection) e verificação de rollback ensaiado (`docs/runbooks/gameday/seed-data.md`) rodam no job `Seed Data Polish Gates`, bloqueando promoção quando violado.
...
- Execução local: se o backend já estiver ativo (ex.: via Compose), use `ZAP_SKIP_SERVER_START=1` e aponte `ZAP_BASELINE_TARGET` para `/metrics`.
