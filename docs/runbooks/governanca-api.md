...
1. **Contratos OpenAPI**
   ...
   - Execute `pnpm run openapi:lint` (Spectral) e `pnpm run openapi:diff` (Redocly OpenAPI CLI). Ambos DEVEM falhar o pipeline se houver erro.
...
## Ações no Pipeline
- Workflow `ci-contracts.yml` executa Spectral, Redocly OpenAPI CLI (diff) e Pact (com degradação controlada quando ferramentas não estiverem instaladas).
