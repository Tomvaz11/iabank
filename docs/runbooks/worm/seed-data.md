...
## Pré-requisitos
- Variáveis exigidas: `SEEDS_WORM_BUCKET`, `SEEDS_WORM_ROLE_ARN`, `SEEDS_WORM_KMS_KEY_ID`, `SEEDS_WORM_RETENTION_DAYS>=1855` (30 dias + 5 anos; fail-close abaixo disso) e `SEED_ENFORCE_WORM_ON_DRY_RUN=true` somente quando quiser forçar WORM em dry-run/CI.
...
- Falhas comuns:  
  - Retenção < 1855 dias → `worm_retention_too_low` (503).  
...
- Evidências devem permanecer WORM (retenção mínima 1855 dias, modo COMPLIANCE) e referenciar o commit/manifesto usado para a execução.  
